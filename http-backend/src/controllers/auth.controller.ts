...
import type { Signin } from "../schema/auth.schema.js";
...
async function signupController(request: Request, response: Response) {
  ...
  try {
    ...
    return response
      .status(409)
      .json(
        new ApiResponse({ success: false, message: err.error, sc: err.sc }),
      );
  }
...
async function signinController(request: Request, response: Response) {
  const data: Signin = request.body;
  ...
  try {
    ...
    const isPasswordMatched = await unHashPassword(
      data.password,
      user.password,
    );
    ...
    if (err.sc === 404) {
      return response
        .status(404)
        .json(
          new ApiResponse({ success: false, message: err.message, sc: 404 }),
        );
    } else {
      return response.status(500).json(
        new ApiResponse({
